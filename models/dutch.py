...
dutch_aug_train = get_aug_pandas_df("./data/nld/train.txt")
#load dev file
dutch_reg_dev = get_reg_pandas_df("./data/nld/dev.txt")
dutch_aug_dev = get_aug_pandas_df("./data/nld/dev.txt")
#load test file
dutch_reg_test = get_reg_pandas_df("./data/nld/test.txt")
dutch_aug_test = get_aug_pandas_df("./data/nld/test.txt")
...
dutch_reg_dataset["dev"] = Dataset.from_pandas(dutch_reg_dev)
dutch_aug_dataset["dev"] = Dataset.from_pandas(dutch_aug_dev)
dutch_reg_dataset["test"] = Dataset.from_pandas(dutch_reg_test)
dutch_aug_dataset["test"] = Dataset.from_pandas(dutch_aug_test)
...
#now we train
reg_trainer = Trainer(reg_model, args = reg_training_args, train_dataset=dutch_tokenized_reg_dataset["train"],
                      eval_dataset=dutch_tokenized_reg_dataset["dev"], data_collator=data_collator, compute_metrics=compute_metrics)
reg_trainer.train()
print(f"Baseline F1: {reg_trainer.evaluate(eval_dataset=dutch_tokenized_reg_dataset['dev'])['eval_f1'] * 100:0.2f}")

aug_trainer = Trainer(aug_model, args= aug_training_args, train_dataset=dutch_tokenized_aug_dataset["train"],
                      eval_dataset=dutch_tokenized_aug_dataset["dev"], data_collator=data_collator, compute_metrics=compute_metrics)
aug_trainer.train()
print(f"Aug F1: {aug_trainer.evaluate(eval_dataset=dutch_tokenized_aug_dataset['dev'])['eval_f1'] * 100:0.2f}")
