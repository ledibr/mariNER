...
- **load_data.py**: This file contains the ```AugmentedDataLoader``` class, which loads datasets from HuggingFace based on user input and performs relevant preprocessing for data augmentation.
...
- *logs_final*: This directory contains the log output from running experiments on the cluster for final test set results.
  - **all_langs_46217.log**: This file contains original/uppercase/lowercase test set results for baseline and augmented models trained and tested on CoNLL03, CoNLL02, GermEval 2014, TurkuNLP, and MasakhaNER 2.0 Zulu.
